...
def filter_data(data):
    ...
    filtered_data = {
    "Domain": data.get("address", {}),
        "Detection Count": data.get("lookup_results", {}).get("detected_by", 0),
        "Sources": [
            {
...
def fetch_data(domain):
    try:
        response = requests.get(f'https://api.metadefender.com/v4/domain/{domain}', headers=headers)
